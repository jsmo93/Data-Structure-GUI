...
(define (make-target table)
  (make-bitmap (* 150 (count-cols table)) (* 40 (count-rows table))))
...
(define (draw-node dc path row col)
  (send dc draw-path path (* 150 col) (* 40 row)))
  ...
  (if (number? data-str)
      (begin 
         ...
         (cond ((= (string-length data-str) 1) 
                (send dc draw-path data (* 150 col) (* 40 row))
                (send dc draw-text data-str (+ 3 (* 150 col)) (- (* 40 row) 1)))
               ((< (string-length data-str) 15)
                (send dc draw-rectangle (* 150 col) (* 40 row) (+ 2 (* 10 (string-length data-str))) 16)
                (send dc draw-text data-str (+ 1 (* 150 col)) (- (* 40 row) 1)))
               (else
                (begin (send dc draw-rectangle (* 150 col) (* 40 row) 148 16))
                (send dc draw-text (substring data-str 0 13) (+ 1 (* 150 col)) (- (* 40 row) 1))
                (send dc draw-text ".." (+ 135 (* 150 col)) (- (* 40 row) 1))))
